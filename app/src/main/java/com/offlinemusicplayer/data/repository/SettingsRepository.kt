...
class SettingsRepository(private val settingsDao: SettingsDao) {
    ...
    suspend fun getFts5Enabled(): Boolean {
        return settingsDao.getSetting(SettingKeys.FTS5_ENABLED)?.value?.toBoolean() ?: true
    }

    fun getFts5EnabledFlow(): Flow<Boolean> {
        return settingsDao.getSettingFlow(SettingKeys.FTS5_ENABLED)
    ...
    suspend fun getShuffleMode(): Boolean {
        return settingsDao.getSetting(SettingKeys.SHUFFLE_MODE)?.value?.toBoolean() ?: false
    }
    ...
    suspend fun getRepeatMode(): Int {
        return settingsDao.getSetting(SettingKeys.REPEAT_MODE)?.value?.toIntOrNull() ?: 0
    }
